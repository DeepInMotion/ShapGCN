...
class ReinforceController(BaseController):
    ...
    def update(self, rollouts: list, metric: str) -> None:
        """
        Perform update step of REINFORCE and sample from replay memory.
        :param rollouts:
        :param metric: metric to optimize in rollouts[2]
        :return:
        """
        ...
        rewards = [i[2][metric] for i in rollouts]
        # exponentiation rewards
        ...
        if self.replay_mem:
            ...
            count = self.replay_memory.push(rollouts_replay_push, metric)
