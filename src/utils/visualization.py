...
def visualize_cp_skeleton(data):
    ...
    def update(frame):
        ...
        plt.scatter(x_data[frame, :], y_data[frame, :], c='blue', marker='o')
        plt.title(f'Time Frame {frame}')
        plt.xlabel('Normalized x')
        plt.ylabel('Normalized y')
        plt.grid(True)

    def plot_skeleton(x, y, time_frame):
        plt.figure(figsize=(8, 6))
        plt.scatter(x[time_frame, :], y[time_frame, :], c='blue', marker='o')
        plt.title(f'Skeleton at Time Frame {time_frame}')
        plt.xlabel('Normalized x')
        plt.ylabel('Normalized y')
        plt.grid(True)
        plt.show()
    ...
    # Create an animation
    ani = animation.FuncAnimation(fig, update, frames=x_data.shape[0], repeat=True, interval=100)
