...
from src.xai.xai_utils import ShapHandler, plot_skel_ntu, plot_skel_cp, CP_JOINT_NAMES_29, NTU_JOINT_NAMES
...
class ShapVisualizer:
    ...
    def visualize_ntu_shap(self, paths) -> None:
        """
        ...
        shap_values_ntu = self.shap_handler.load_shap_values_ntu(paths, target_class)
    ...
    def visualize_cp_shap(self, paths) -> None:
        """
        ...
        shap_values_cp, shap_names_cp, shap_labels_cp = self.shap_handler.load_shap_values_cp(paths)
        ...
        if single_infant:
            # maybe random here
            class_0_names = shap_names_cp[self.shap_handler.class_0_indices]
            class_1_names = shap_names_cp[self.shap_handler.class_1_indices]
            ...
            self.shap_handler.class_0_indices = np.arange(0, len(indices_class_0), 1, dtype=int)
            self.shap_handler.class_1_indices = np.arange(len(self.shap_handler.class_1_indices),
                                                          len(self.shap_handler.class_1_indices) + len(indices_class_1),
                                                          1, dtype=int)
            self.class_0_indices = indices_class_0
            self.class_1_indices = indices_class_1
        else:
            logging.info("Visualizing all infants...")
            self.class_0_indices = self.shap_handler.class_0_indices
            self.class_1_indices = self.shap_handler.class_1_indices
    ...
    @staticmethod
    def split_shaps_features_ntu(feature_values, shap_values):
    ...
    @staticmethod
    def split_shaps_features_cp(feature_values, shap_values):
        x_shaps = {k: v for k, v in shap_values.items() if '_x' in k}
        y_shaps = {k: v for k, v in shap_values.items() if '_y' in k}
        x_feature = {k: v for k, v in feature_values.items() if '_x' in k}
        y_feature = {k: v for k, v in feature_values.items() if '_y' in k}
    ...
    def average_cp_feature_values(self, windows=False, time=False, body=False) -> tuple[dict, dict]:
        ...
        feature_values_class_1 = self.shap_handler.extract_feature_values_cp(feature_data_1)
        feature_values_class_0 = self.shap_handler.extract_feature_values_cp(feature_data_0)
    ...
    def average_ntu_feature_values(self, indices, windows=False, time=False, body=False) -> dict:
        ...
        # sort based on features
        feature_values_class = self.shap_handler.extract_feature_values_ntu(feature_data)
    ...
    def average_ntu_shap_values(self, shap_values_cp, windows=False, time=False, body=False) -> dict:
        ...
        :return:
        """
        shap_values_ntu = self.shap_handler.extract_feature_values_ntu(shap_values_cp)
    ...
    def average_cp_shap_values(self, shap_values_cp, windows=False, time=False, body=False) -> tuple[dict, dict]:
        ...
        shap_values_dict = self.shap_handler.extract_feature_values_cp(shap_values_cp)

        # Loop through the SHAP values and get class-specific values
        for key, value in shap_values_dict.items():
            shap_values_class_0[key], shap_values_class_1[key] = self.shap_handler.get_class_values(value)
    ...
    def average_helper(self, values: dict, windows: bool, time: bool, body: bool) -> dict:
        ...
        if windows:
            ...
            values = {key: self.shap_handler.average_values_index(value) for key, value in values.items()}

        if time:
            logging.info("Averaged values over time....")
            values = {key: self.shap_handler.average_values_time(value, window_averaged=windows)
                      for key, value in values.items()}

        if body:
            logging.info("Averaged values over body groups.....")
            values = {key: self.shap_handler.average_values_body_groups(value, self.body_parts)
                      for key, value in values.items()}
